...
load_distro_specifics(){
  ...
  export RAGUEL_PACKAGE_UNINSTALL='apt-get -y uninstall '
  elif [ -f /etc/gentoo-release ]; then
    export RAGUEL_DISTRO_TYPE='gentoo'
    export RAGUEL_PACKAGE_INSTALL='emerge '
  export RAGUEL_PACKAGE_UNINSTALL='emerge -X '
  ...
  export RAGUEL_PACKAGE_INSTALL='pkg_add -X  '
  elif [ -f /etc/SuSE-release ] ; then
    export RAGUEL_DISTRO_TYPE='slackware'
    export RAGUEL_PACKAGE_INSTALL='zypper install -y '
  export RAGUEL_PACKAGE_INSTALL='zypper uninstall -y '
  else
